...
import { Mail, Lock, User, ArrowRight, Sun, Moon, Pickaxe, AtSign } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const handleRegisterSubmit = async () => {
    ...
    if (Object.keys(errs).length > 0) {
      setFieldErrors(errs);
      setGlobalError("กรุณากรอกข้อมูลให้ครบถ้วนและถูกต้อง");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] dark:bg-[#0a0c0f] flex flex-col items-center justify-center p-4 transition-colors duration-300">
      ...
      <div className="w-full max-w-md p-8 rounded-2xl border border-gray-200 dark:border-[#1e2330] shadow-sm dark:shadow-[0_8px_30px_rgb(0,0,0,0.4)] bg-white dark:bg-[#111318] relative z-10 my-8">
        <div className="text-center mb-8">
          <Link href="/" className="inline-block mb-6">
            <div className="w-10 h-10 mx-auto rounded-lg bg-gradient-to-br from-[#10d97e] to-[#0ea865] flex items-center justify-center text-white pb-1 shadow-[0_0_15px_rgba(16,217,126,0.5)]">
              <span className="font-syne font-bold text-lg"><Pickaxe size={18} /></span>
            </div>
          </Link>
          <h1 className="font-syne text-2xl font-bold text-gray-900 dark:text-[#e8ecf4] mb-2 tracking-tight">สร้างบัญชีฟรี</h1>
          <p className="text-sm text-gray-500 dark:text-[#8892a4]">เริ่มต้นใช้งานได้ทันที ไม่ต้องใส่ข้อมูลบัตรเครดิต</p>
        </div>
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleRegisterSubmit(); }} className="space-y-4" noValidate>
          <div>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                <AtSign size={18} className={fieldErrors.username ? "text-red-500" : "text-gray-400 dark:text-[#4a5568]"} />
              </div>
              <input
                type="text"
                name="username"
                placeholder="Username (เช่น steve_craft)"
                value={formData.username}
                onChange={handleChange}
                className={`w-full pl-11 pr-4 py-3 rounded-xl bg-gray-50 dark:bg-[#0a0c0f] border focus:outline-none transition-all text-gray-900 dark:text-[#e8ecf4] placeholder-gray-400 dark:placeholder-[#4a5568] ${fieldErrors.username
            ...
            {fieldErrors.username && <p className="text-red-500 text-xs mt-1.5 ml-1 font-medium">{fieldErrors.username}</p>}
          </div>

          <div>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                <Mail size={18} className={fieldErrors.email ? "text-red-500" : "text-gray-400 dark:text-[#4a5568]"} />
              </div>
              <input
                type="email"
                name="email"
                ...
                onChange={handleChange}
                className={`w-full pl-11 pr-4 py-3 rounded-xl bg-gray-50 dark:bg-[#0a0c0f] border focus:outline-none transition-all text-gray-900 dark:text-[#e8ecf4] placeholder-gray-400 dark:placeholder-[#4a5568] ${fieldErrors.email
                  ? "border-red-500/50 focus:border-red-500/50 focus:ring-1 focus:ring-red-500/50"
                  : "border-gray-200 dark:border-[#1e2330] focus:border-[#10d97e]/50 focus:ring-1 focus:ring-[#10d97e]/50"
                  }`}
              />
            </div>
            {fieldErrors.email && fieldErrors.email !== " " && <p className="text-red-500 text-xs mt-1.5 ml-1 font-medium">{fieldErrors.email}</p>}
          </div>
          ...
          <div>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                <Lock size={18} className={fieldErrors.confirm ? "text-red-500" : "text-gray-400 dark:text-[#4a5568]"} />
              </div>
              <input
                type="password"
                name="confirm"
                ...
                onChange={handleChange}
                className={`w-full pl-11 pr-4 py-3 rounded-xl bg-gray-50 dark:bg-[#0a0c0f] border focus:outline-none transition-all text-gray-900 dark:text-[#e8ecf4] placeholder-gray-400 dark:placeholder-[#4a5568] ${fieldErrors.confirm
                  ? "border-red-500/50 focus:border-red-500/50 focus:ring-1 focus:ring-red-500/50"
                  : "border-gray-200 dark:border-[#1e2330] focus:border-[#10d97e]/50 focus:ring-1 focus:ring-[#10d97e]/50"
                  }`}
              />
            </div>
            {fieldErrors.confirm && fieldErrors.confirm !== " " && <p className="text-red-500 text-xs mt-1.5 ml-1 font-medium">{fieldErrors.confirm}</p>}
          </div>
          ...
          <div className="pt-2">
            ...
            >
              <span>{loading ? "กำลังดำเนินการ..." : "สร้างบัญชีฟรี"}</span>
        ...
        <p className="mt-8 text-center text-sm text-gray-500 dark:text-[#8892a4]">
          ...
          <Link href="/auth/login" className="text-[#10d97e] font-medium hover:underline">
            เข้าสู่ระบบเลย
          </Link>
        </p>
      </div>
